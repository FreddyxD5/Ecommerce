...
export const get_total = () => async dispatch => {
    if (localStorage.getItem('access')) {
        try{
            const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/cart/get-total`, config_auth)
            if (res.status === 200){
                dispatch({
...
export const remove_item = item => async dispatch=>{
    if (localStorage.getItem('access')){
        ...
        if(localStorage.getItem('cart')){
            ...
            cart.map(car_item=>{
                if(car_item.product.id.toString() !== item.product.id.toString()){
                    new_cart.push(new_cart)
                }
            })
...
export const empty_cart = () => async dispatch =>{
    if(localStorage.getItem('access')){
        try{
            const res = await axios.delete(`${process.env.REACT_APP_API_URL}/api/cart/empty-car`, config_auth)
            if(res.status ===200){
                dispatch({
...
export const sync_cart = () => async dispatch =>{
    const config={
        headers:{
            'Content-Type':'application/json',
            'Accept':'application/json',
